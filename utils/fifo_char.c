...
bool fifo_reset(Fifo_array_t *const instance) {
  bool res = false;
  if ((NULL != instance) && (0u < instance->fifoState.size) &&
      (true == instance->initDone)) {
...
bool fifo_push_array(Fifo_array_t *instance, char *const inArr,
                     uint16_t arrLen) {
  bool res = true;
#if DEBUG_FIFO_CHAR
...
bool fifo_pull_array(Fifo_array_t *instance, char *const outArr,
...
bool fifo_peek_array(Fifo_array_t *instance, char *outArr,
                     uint16_t *const outLen) {
  bool res = false;
  if (NULL != instance) {
