...
bool tcan4550_parse_reg_status(uint32_t reg_val) {
    ...
    if(reg.write_overflow) {
        LOG_DEBUG(CAN,"bit %u: SPI write sequence had continue requests after the data transfer was completed %u" CRLF, 19,
                  reg.write_overflow);
    }
    ...
    if(reg.read_overflow) {
        LOG_DEBUG(CAN,"bit %u: SPI read sequence had continue requests after the data transfer was completed %u" CRLF, 17,
                  reg.read_overflow);
    }
    ...
    if(reg.write_fifo_available) {
        LOG_DEBUG(CAN,"bit %u: write fifo empty entries is greater than or equal to the write_fifo_threshold %u" CRLF, 5,
    ...
    if(reg.internal_access_active) {
        LOG_DEBUG(CAN,"bit %u: Internal Multiple transfer mode access in progress %u" CRLF, 3, reg.internal_access_active);
    }
...
bool tcan4550_parse_reg_interrupt_flags(uint32_t reg_val) {
    ...
    tCanRegIntFl_t reg;
    memcpy(&reg, &reg_val, 4);
    LOG_DEBUG(CAN,"Addr 0x%04X %s 0x%08x 0b%s" CRLF, ADDR_IF, tcan4550_get_reg_name(ADDR_IF), reg_val, utoa_bin32(reg_val));
