...
bool diag_page_tasks(ostream_t* stream) {
    ...
    static const table_col_t cols[] = {{22, "Task name"},
                                       {8, "Calls/s"},
                                       {6, "CPU[%]"},
                                       {12, "Tavg[us]"},
                                       {12, "Tmin[us]"},
    ...
    for(core = 0; core < ARRAY_SIZE(data) - 1U; core++) {
        ...
        for(id = 0; id < count[core]; id++) {
            if (0 != data[core][id].start_count) {
                uint16_t cpu_e3 = data[core][id].run_time_total * 1000 / total_time_us;
                oprintf(stream,
                ...
                oprintf(stream, " %11" PRIu64" " TABLE_SEPARATOR CRLF, data[core][id].run_time_total);
            }
        }
