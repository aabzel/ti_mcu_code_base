...
static void table_row(ostream_t *row_ostream, const table_col_t row_cols[],
                      int16_t row_col_count,
                      const table_row_format_t *row_format);
static void table_row_hdr(ostream_t *row_hdr_ostream,
                          const table_col_t row_hdr_cols[],
                          int16_t row_hdr_col_count,
...
void table_row_top(ostream_t *row_top_ostream, const table_col_t row_top_cols[],
  ...
  static const table_row_format_t row_top_format = {
      TABLE_TOP_LEFT, TABLE_TOP_SEPARATOR, TABLE_TOP_RIGHT, TABLE_TOP_FILL};
...
void table_row_separator(ostream_t *row_separator_ostream,
  ...
  static const table_row_format_t row_separator_format = {
      TABLE_SEP_LEFT, TABLE_SEP_SEPARATOR, TABLE_SEP_RIGHT, TABLE_SEP_FILL};
  table_row(row_separator_ostream, row_separator_cols, row_separator_col_count,
            &row_separator_format);
}
...
void table_row_bottom(ostream_t *row_bottom_ostream,
  ...
  static const table_row_format_t row_bottom_format = {
      TABLE_BTM_LEFT, TABLE_BTM_SEPARATOR, TABLE_BTM_RIGHT, TABLE_BTM_FILL};
  table_row(row_bottom_ostream, row_bottom_cols, row_bottom_col_count,
            &row_bottom_format);
}
...
static void table_row_hdr(ostream_t *row_hdr_ostream,
  ...
  for (hdr_col_index = 0; hdr_col_index < cnt_row_hdr_col_count;
       hdr_col_index++) {
    ...
    if (row_hdr_cols[hdr_col_index].width < text_len) {
      oputs_len(row_hdr_ostream, row_hdr_cols[hdr_col_index].name,
                (int32_t)row_hdr_cols[hdr_col_index].width);
    } else {
...
void table_row_header(ostream_t *row_header_ostream,
  ...
  static const table_row_format_t row_header_format = {
      TABLE_LEFT, TABLE_SEPARATOR, TABLE_RIGHT, TABLE_FILL};
  table_row_hdr(row_header_ostream, row_header_cols, row_header_col_count,
                &row_header_format);
}
