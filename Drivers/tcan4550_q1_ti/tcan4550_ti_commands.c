...
bool tcan4550_ti_send( uint16_t id, uint64_t data64){
    ...
    TCAN4x5x_MCAN_WriteTXBuffer(0, &header, &un64.u8[0]);  // This function actually writes the header and data payload to the TCAN's MRAM in the specified TX queue number. It returns the bit necessary to write to TXBAR,
