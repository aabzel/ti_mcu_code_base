...
static bool init_uart(uint8_t uart_num) {
    ...
    huart[uart_num].rx_buff = rx_buff6;
    huart[uart_num].rx_buff_size = sizeof(rx_buff6);
    ...
    uart_send(uart_num, "uart", 5, true);
    return res;
}
...
#endif
    if(uart_handle->Instance == USART6) {
