...
#define EXT_WDT_TEST                                                           \
  SHELL_CMD("wd_test_hw", "hwd_test", cmd_wd_test_hw,                          \
            "Stop board (for hardware watchdog test)"),
#else
#define EXT_WDT_TEST
#endif
...
#define DEFAULT_COMMANDS                                                       \
  SHELL_CMD("help", "h", cmd_help, "Print list of shell commands"),            \
      ...
      SHELL_CMD("sysinfo", "si", cmd_sysinfo,                                  \
                "Print information about threads & OS"),                       \
      SHELL_CMD("soft_reboot", "reboot", cmd_soft_reboot, "Reboot board"),     \
      SHELL_CMD("wd_test", "wd_test", cmd_wd_test,                             \
                "Stop board (for watchdog test)"),                             \
      EXT_WDT_TEST

#else

#define TEST_DEFAULT_COMMANDS                                                  \
  SHELL_CMD("launch_function", "lfun", cmd_launch_function,                    \
            "Launch any function by address in ROM"),                          \
      SHELL_CMD("try_stack", "tstk", cmd_try_stack, "Explore stack RAM"),      \
      SHELL_CMD("repeat", "rpt", cmd_repeat,                                   \
                "Repeat any command N time with period"),
